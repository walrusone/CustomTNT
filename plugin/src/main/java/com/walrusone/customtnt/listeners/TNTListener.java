...
public class TNTListener implements Listener
{
    ...
    public void onExplosion(final BlockExplodeEvent event) {
    	if (CustomTNT.getTntHandler().getLuckyExplosions().contains(event.getBlock().getLocation())) {
    		...
    		for (Block block: event.blockList()) {
    			if (block.getType().equals(CustomTNT.getNMS().getSpawerMaterial())) {
    				...
    				if (randomNum <= ((LuckyTNT) CustomTNT.getTntHandler().getExplosionType(TNTType.LUCKY)).getOdds()) {
						...
						ItemStack mobSpawner = new ItemStack(CustomTNT.getNMS().getSpawerMaterial(), 1);
    ...
    public void onExplosivePrimed(final ExplosionPrimeEvent event) {
		if (event.getEntityType() == EntityType.PRIMED_TNT) {
			...
			if (primedTNT.hasMetadata("TNTType")) {
				...
				if(CustomTNT.getNMS().allowExplosion(event.getEntity().getLocation(), CustomTNT.get())) {
					new BukkitRunnable() {
						public void run() {
    ...
    public void onBlockPlaced(final BlockPlaceEvent e) {
        if (e.getItemInHand().getType() == Material.TNT) {
        	if (CustomTNT.getNMS().hasKey(e.getItemInHand(), "TntType")) {
				String type = CustomTNT.getNMS().getKeyType(e.getItemInHand(), "TntType");
				if (CustomTNT.getNMS().allowPlacement(e.getBlock().getLocation(), e.getPlayer(), CustomTNT.get())) {
    ...
    public void onDispense(final BlockDispenseEvent event) {
        if (event.getItem().getType() == Material.TNT) {
			if (CustomTNT.getNMS().hasKey(event.getItem(), "TntType")) {
				String type = CustomTNT.getNMS().getKeyType(event.getItem(), "TntType");
    ...
    public void onPlayerInteract(PlayerInteractEvent e) {
		ItemStack item = CustomTNT.getNMS().getMainHandItem(e.getPlayer());
        ...
        if ((e.getAction().equals(Action.RIGHT_CLICK_AIR) || e.getAction().equals(Action.RIGHT_CLICK_BLOCK)) && item.getType().equals(Material.TNT)) {
			if (CustomTNT.getNMS().hasKey(item, "TntType")) {
				String type = CustomTNT.getNMS().getKeyType(item, "TntType");
        		if(e.getPlayer().hasPermission(CustomTNT.getTntHandler().getExplosionType(TNTType.valueOf(type.toUpperCase())).getPermission())) {
        			if (CustomTNT.getTntHandler().getExplosionType(TNTType.valueOf(type.toUpperCase())).isThrowable()) {
            			...
            			if (e.getAction().equals(Action.RIGHT_CLICK_AIR)) {
            				if (type.equalsIgnoreCase("suicide")) {
                				...
                				int amount = CustomTNT.getNMS().getMainHandItem(player).getAmount();
                				if (amount > 1) {
									CustomTNT.getNMS().getMainHandItem(player).setAmount(amount - 1);
                				} else {
									CustomTNT.getNMS().setMainHandItem(player, new ItemStack(Material.AIR, 1));
                				}
    ...
    public void onPlayerPickupItem(PlayerPickupItemEvent e) {
			...
			if (e.getItem().getItemStack().getType() == Material.TNT) {
				if (CustomTNT.getNMS().hasKey(e.getItem().getItemStack(), "TntType")) {
					String type = CustomTNT.getNMS().getKeyType(e.getItem().getItemStack(), "TntType");
	...
	public void onPlayerJoin(PlayerJoinEvent e) {
    	CustomTNT.getNMS().discoverRecipes(e.getPlayer());
	}
