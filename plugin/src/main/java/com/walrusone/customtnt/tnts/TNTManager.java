...
public class TNTManager {
	...
	private String getParticle(String key) {
		return config.getString("tntTypes." + key + ".particleType");
	}
	...
	private void addRecipe(String key) {
		CustomTNT.getNMS().addRecipe(CustomTNT.get(), key, config, this.getExplosionType(TNTType.valueOf(key.toUpperCase())).getItem());
	}
